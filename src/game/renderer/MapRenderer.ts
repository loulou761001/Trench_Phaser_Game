...
export class MapRenderer {
	...
	private drawTrench(x: number, y: number) {
		...
		const connectedSides = {
			...
			left:
				x > 0 && trenchTiles.includes(this.objectTiles[y][x - 1] as TileType),
			top:
				y > 0 && trenchTiles.includes(this.objectTiles[y - 1][x] as TileType),
			bottom:
		...
		const groundSprite =
			this.objectTiles[y][x]?.type === "parapet" ? "parapet" : "wood_planks";
		this.scene.add
