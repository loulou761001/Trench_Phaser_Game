	...
	private drawTrench(x: number, y: number) {
		const connectedSides = {
			right:
				...
				this.objectTiles[y][x + 1] === TileTypes.TRENCH,
			left: x > 0 && this.objectTiles[y][x - 1] === TileTypes.TRENCH,
			top: y > 0 && this.objectTiles[y - 1][x] === TileTypes.TRENCH,
			bottom:
				y < this.objectTiles.length - 1 &&
				this.objectTiles[y + 1][x] === TileTypes.TRENCH,
		};
