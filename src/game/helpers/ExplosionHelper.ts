...
import { areInSameTrench, getGridTerrain, WorldCoordsType } from "./MapHelper.ts";
...
export function generateExplosion(explosionRange: number, explosionLines: number, lethality: number, targetPos: WorldCoordsType, doGenerateCrater: boolean = false) {
  ...
  const isExplosionInTrench = !!(gridTerrain && trenchTerrains.includes(gridTerrain))
  for (let i = 0; i < explosionLines; i++) {
    ...
    if (isExplosionInTrench) {
      ...
      for (const point of points) {
        ...
        const pointGridTerrain = getGridTerrain(pointGridCoods.x, pointGridCoods.y)
    ...
    if (closestHit) {
      const sameTrench = isExplosionInTrench && areInSameTrench(
