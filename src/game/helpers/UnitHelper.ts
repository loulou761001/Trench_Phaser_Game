...
export function getTacticalRole(unit: Unit): "fire" | "advance" {
	const nonSuppressedEnemiesInRange = GameState.unitManager
		...
		.filter((u) => {
			...
			return (
				dist <= unit.getEquippedWeapon().range && u.stance !== "suppressed"
			);
		});
	...
	if (terrain === TileTypes.TRENCH)
		advanceChance -= 0.3; // safer, more likely to fire
	else if (terrain === TileTypes.CRATER)
		advanceChance -= 0.1; // semi-safe
...
export function getCoverBonus(unit: Unit, inSameTrench = false) {
	...
	if (
		(currentTile === TileTypes.TRENCH || currentTile === TileTypes.PARAPET) &&
		inSameTrench
	) {
		coverBonus -= 2;
	}
