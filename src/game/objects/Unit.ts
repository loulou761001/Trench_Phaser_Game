		...
		this.scene.time.delayedCall(aimTimeMs, () => {
			...
			const spreadAngle = Phaser.Math.DegToRad(
				baseAngle *
					...
					this.weapons[this.equippedWeapon].accuracy *
			...
			const bulletLine = new Phaser.Geom.Line(
				this.x,
				this.y,
				this.x + Math.cos(finalAngle) * this.weapons[this.equippedWeapon].range,
				this.y + Math.sin(finalAngle) * this.weapons[this.equippedWeapon].range,
			);
			...
			if (closestHit && closestHit.target.team === this.team) {
				this.weapons[this.equippedWeapon].activeState.canFire = true;
				this.weapons[this.equippedWeapon].activeState.fireCooldown = null;
				return;
			}
			if (closestHit) {
			...
			this.weapons[this.equippedWeapon].drawTracer(
				bulletLine.x1,
				bulletLine.y1,
				bulletLine.x2,
				bulletLine.y2,
			);
			this.weapons[this.equippedWeapon].playSound();

			this.weapons[this.equippedWeapon].activeState.isFirstShot = false;
			this.weapons[this.equippedWeapon].activeState.roundsFired++;
		});
	}
