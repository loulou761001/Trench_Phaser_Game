...
import {
	...
	detectBulletHit,
} from "../helpers/ShotCalculationsHelper.ts";
import {
	checkWeaponCooldown,
	getAimTime,
	getCoverBonus,
...
export class Unit extends Phaser.GameObjects.Sprite {
	...
	shadowCircle: Phaser.GameObjects.Arc;
	...
	constructor(
		...
		this.originalSpeed = speed ?? 80;
		this.speed = this.originalSpeed;
		this.underCircle = this.scene.add
			.circle(this.x, this.y, UNIT_SIZE / 2.5, 0xffff00, 0)
			.setDepth(this.depth);
		this.shadowCircle = this.scene.add
			.circle(this.x, this.y, UNIT_SIZE / 4, 0x000000, 0.15)
	...
	update(delta: number) {
		...
		this.shadowCircle.x = this.x;
		this.shadowCircle.y = this.y;
		...
		if (this.unitAi) {
			...
			this.setMoving(this.path.length > 0 && this.stance !== "suppressed");
	...
	private move(delta: number) {
		...
		if (dist < 2) {
			...
			const speedModifier = this.getCurrentTerrain().speedMultiplier;
	...
	throwGrenade() {
		...
		const canFireGrenade =
			...
			(Math.random() < 0.2 || currentTarget.isInCover());
		if (!canFireGrenade) return;
		generateExplosion(120, 24, 0.8, {
	...
	fireShot() {
		...
		const target = this.unitAi.currentState.target;
		if (!weapon.activeState.canFire || !target) return;

		// Maybe move grenade logic OUT of here
		this.tryThrowGrenade();

		weapon.activeState.canFire = false;

		const aimTimeMs = weapon.activeState.isFirstShot
		...
		this.scene.time.delayedCall(aimTimeMs, () => {
			...
			this.shootAtTarget(weapon, target);
		});
	}

	private shootAtTarget(weapon: Weapon, target: Unit) {
		const bulletLine = calculateBulletTrajectory(weapon, target, this);
		const { closestHit, nearMisses } = detectBulletHit(bulletLine, this, true);

		if (closestHit) {
			applyHit(this, weapon, closestHit, bulletLine);
		}

		applyNearMissMorale(nearMisses);
		...
		weapon.activeState.isFirstShot = false;
		weapon.activeState.roundsFired++;
	}
	...
	private tryThrowGrenade() {
		...
		if (inRange && (Math.random() < 0.2 || target.isInCover())) {
			generateExplosion(120, 24, 0.8, { worldX: target.x, worldY: target.y });
			grenade.totalAmmo--;
		}
	}
	...
	setMoving(isMoving: boolean) {
		this.unitAi.currentState.isMoving = isMoving;
	}
	...
	isInCover() {
		return (
			[TileTypes.TRENCH, TileTypes.PARAPET].includes(
